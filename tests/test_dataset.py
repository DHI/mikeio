...
def test_min_max_mean_z_dataset_nan() -> None:
    """Test min/max/mean(axis='z') on a Dataset with NaN and two items.

    Uses the NaN test file where:
      Column 0: values [1,2,3,4,5], all active
      Column 1: values [NaN,20,30,40,50], bottom z-layer inactive
    Second item is first item multiplied by 2.
    """
    ds = mikeio.read("tests/testdata/sigma_z_2elem_5layers_nan.dfsu")
    da = ds[0]

    # Create second item as doubled values
    da2 = da.copy()
    da2.values = da.values * 2
    da2.item = ItemInfo("Double")
    ds2 = Dataset([da, da2])

    # min(axis="z")
    ds_min = ds2.min(axis="z")
    assert len(ds_min) == 2
    assert not ds_min[0].geometry.is_layered
    assert ds_min[0].geometry.n_elements == 2
    # Item 0: Col 0 min=1, Col 1 min=20 (NaN excluded)
    assert np.isclose(ds_min[0].to_numpy()[0, 0], 1.0)
    assert np.isclose(ds_min[0].to_numpy()[0, 1], 20.0)
    # Item 1 (doubled): Col 0 min=2, Col 1 min=40
    assert np.isclose(ds_min[1].to_numpy()[0, 0], 2.0)
    assert np.isclose(ds_min[1].to_numpy()[0, 1], 40.0)

    # max(axis="z")
    ds_max = ds2.max(axis="z")
    assert len(ds_max) == 2
    # Item 0: Col 0 max=5, Col 1 max=50
    assert np.isclose(ds_max[0].to_numpy()[0, 0], 5.0)
    assert np.isclose(ds_max[0].to_numpy()[0, 1], 50.0)
    # Item 1 (doubled): Col 0 max=10, Col 1 max=100
    ...
    assert np.isclose(ds_mean[0].to_numpy()[0, 0], 3.0)
    assert np.isclose(ds_mean[0].to_numpy()[0, 1], 35.0)
    # Item 1 (doubled): Col 0 mean=6.0, Col 1 mean=70.0
    assert np.isclose(ds_mean[1].to_numpy()[0, 0], 6.0)
    assert np.isclose(ds_mean[1].to_numpy()[0, 1], 70.0)
