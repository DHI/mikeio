...
def test_dfsu_dataset_has_no_custom_blocks() -> None:
    ...
    Exposing it would let a user mutate node/element counts, and it becomes wrong
    after any spatial subset. It is regenerated by mikecore on write.
    """
...
def test_write_dfsu_warns_and_drops_custom_blocks(tmp_path: Path) -> None:
    ...
    with pytest.warns(UserWarning, match="not dfsu"):
        ds.to_dfs(fp)
