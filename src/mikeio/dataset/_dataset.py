    ...
    * ds.itemA - returns the "itemA" DataArray
    ...
    def extract_vertical(
        ...
        *,
        ...
        (in the mesh coordinate system) to produce a 2D vertical section for
        ...
        >>> section = ds.extract_vertical(xs=[350000, 360000], ys=[6150000, 6160000])

        """
        zn = self[0]._zn
        geometry, zn_out, values = _extract_vertical(
            geometry=self.geometry,
            zn=zn,
            values=[da.to_numpy() for da in self],
            mode=mode,
            xs=xs,
            ys=ys,
            layer_min=layer_min,
        ...
        das = [
            DataArray(
                ...
                time=self.time,
                geometry=geometry,
                item=deepcopy(da.item),
                zn=zn_out,
            )
            for i, da in enumerate(self)
        ]
        return Dataset(das)
