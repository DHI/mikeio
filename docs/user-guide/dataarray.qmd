...
See the [DataArrayPlotter](`mikeio.dataset.DataArrayPlotter`) API for details.
...
* [`dropna()`](`mikeio.DataArray.dropna`) - Remove time steps where the entire field is NaN
* [`fillna()`](`mikeio.DataArray.fillna`) - Fill missing values with a constant value
* [`squeeze()`](`mikeio.DataArray.squeeze`) - Remove axes of length 1. Deprecated, and removed in v4.0; use `isel()` to select the indices you want.
