...
lon_mid = (transect_lon[:-1] + transect_lon[1:]) / 2
lat_mid = (transect_lat[:-1] + transect_lat[1:]) / 2
dlon = transect_lon[1] - transect_lon[0]
dlat = transect_lat[1] - transect_lat[0]
lon_interfaces = np.concatenate([[transect_lon[0] - dlon / 2], lon_mid, [transect_lon[-1] + dlon / 2]])
lat_interfaces = np.concatenate([[transect_lat[0] - dlat / 2], lat_mid, [transect_lat[-1] + dlat / 2]])

n_nodes_per_col = n_depths + 1

# Build node coordinates: column-major, bottom-to-top
# Each column repeats the same (lon, lat), z goes from deepest to surface
node_x = np.repeat(lon_interfaces, n_nodes_per_col)
node_y = np.repeat(lat_interfaces, n_nodes_per_col)
node_z = np.tile(-z_interfaces[::-1], n_points + 1)
...
# Build all candidate elements on the full grid
pi_grid, li_grid = np.meshgrid(np.arange(n_points), np.arange(n_depths), indexing="ij")
pi_all = pi_grid.ravel()
li_all = li_grid.ravel()
di_all = n_depths - 1 - li_all  # depth index (deepest first in mesh)

# Keep only elements above the seafloor
valid = ~np.isnan(mask[di_all, pi_all])
pi_valid = pi_all[valid]
li_valid = li_all[valid]
di_valid = di_all[valid]
