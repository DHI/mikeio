...
To use the bathymetry as a MIKE 21 domain, the file needs a land value, stored in the `M21_Misc` custom block. Custom blocks belong to the file, not to an item, so they live on a `Dataset`; promote the `DataArray` with `to_dataset()`.
...
See the [Dfs2 user guide](../../user-guide/dfs2.qmd#custom-blocks) for more on custom blocks.
