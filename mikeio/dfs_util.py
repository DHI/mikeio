...
def type_list(search=None):
    ...
    if search is not None:
        search = search.lower()
        items = dict([[key, value] for key, value in items.items() if search
                      in value.lower() or search == value.lower()])
...
def unit_list(type_enum, search=None):
    ...
    if search is not None:
        items = dict([[key, value] for key, value in items.items() if
                        search.lower() in value.lower() or search.lower() == value.lower()])
