...
class Pfs:
    ...
    def PFS_to_dict(SourceFile):
        ...
        LevelNames = [
            ("Level" + str(i) + "Name") for i in range(1, nr_Levels + 1)
        ]  # list comprehension to create the names of the levels.
        columnNames = [
            "PFS",
            "Type",
            "LevelNo",
        ...
        for i in df.index:
            if i == 0:
                ...
                df.loc[i, "LevelNo"] = df.loc[
                    i - 1, "LevelNo"
                ]  # assign level the same as previous
                ...
                df.loc[i, "LevelNo"] = (
                    df.loc[i - 1, "LevelNo"] + 1
                )  # Add +1 to the level when passing a StartSect
                df.loc[i, LevelNames] = df.loc[i - 1, LevelNames]
                df.loc[i, "Level" + str(df.loc[i, "LevelNo"]) + "Name"] = str(
                    df.loc[i, "PFS"][1:-1]
                )
            elif df.loc[i, "Type"] == "EndSect":
                df.loc[i, LevelNames] = df.loc[i - 1, LevelNames]
                df.loc[i, "Level" + str(df.loc[i - 1, "LevelNo"]) + "Name"] = float(
                    "NaN"
                )
        ...
        df["KeyName"] = (
            ...
            + df["Level6Name"]
            + df["Key"]
        )  # This hardcode to 6 levels command is not ideal, need to look for alternative
        ...
        df_dropna = df[["KeyName", "Value"]].dropna(
            subset=["KeyName", "Value"], how="all"
        )  # drop rows where both key and value is NaN
        df_keyvalue_dicts = (
            df_dropna.dropna().drop_duplicates()
        )  # drop any rows with NaN, and remove duplicates. We are left with only the key value pairs at the deepest level of the sub-dicts
        df_dictofdicts = df_dropna["KeyName"][
            df["Value"].isnull()
        ].drop_duplicates()  # the inverse of df_keyvalue_dicts

        for i in enumerate(
            df_dictofdicts
        ):  # We need to print the dicts containing dicts first, otherwise they will write over the keyvalue_dicts. This changes up the pfs file sequence, but tests show that it does not seem to affect how it is read by the Mike engine.
            exec(
                "PFS_dict" + i[1] + "= {}"
            )  # This exec command is not ideal, need to look for alternative

        for i in enumerate(df_keyvalue_dicts["KeyName"].index):
            exec(
                "PFS_dict"
                + df_keyvalue_dicts.loc[i[1], "KeyName"]
                + ' = r"'
                + str(df_keyvalue_dicts.loc[i[1], "Value"])
                + '"'
            )  # This exec command is not ideal, need to look for alternative
        return PFS_dict
